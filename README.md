...
- 断りなくコメント・ドキュメンテーションを削除しないでください。
...
- Undoを行うと、エッジの表示だけが戻り、頂点の表示が戻らない。
- Undoをしても表示だけが戻り、内部的な状態が戻らない。
- Undo後に操作をすると、UNDOしなかったかのように振る舞う（実質Undoが出来ていない）
- Undo/Redo機能のテストが不十分なため、潜在的なバグが存在する可能性が高い。
