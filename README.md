...
編集モード: 「編集モード」ボタンで編集モードをONにします。

ツール選択: 「ライン編集」または「ポリゴン編集」ツールを選択します。

頂点ハンドル表示: 編集対象の線または面の頂点が、小さな円形のハンドルで表示されます。

頂点の選択:

クリック: 頂点ハンドルをクリックすると、その頂点が選択状態になります。

複数選択: Shiftキーを押しながらクリックすることで、複数の頂点を同時に選択できます。

頂点の移動: 選択した頂点ハンドルをドラッグすると、頂点の位置を移動できます。移動中はグリッドにスナップします（グリッド表示がONの場合）。

頂点の追加:

エッジ上をクリック: 線または面のエッジ上をクリックすると、クリックした位置に新しい頂点が追加されます。

頂点の削除:

Deleteキー: 選択した頂点を削除します。

コンテキストメニュー: 頂点ハンドルを右クリックし、コンテキストメニューから「頂点を削除」を選択して削除します。削除操作を行う前に確認ダイアログを表示します。

コンテキストメニュー: 頂点ハンドルを右クリックすると、以下の項目を持つコンテキストメニューが表示されます。
...
グリッド表示: グリッド表示のON/OFFを切り替えるボタンをツールバーに追加します。グリッド表示がONの場合、編集時に頂点がグリッドにスナップします。

アンドゥ・リドゥ機能: 編集操作（頂点の移動、追加、削除）のアンドゥ・リドゥができます。

選択範囲: マウスドラッグで矩形選択範囲を作成し、範囲内の頂点を選択できます。
