...
- **共有頂点**:
    ...
    - 頂点同士の距離を近づけることで作成可能。
        - この距離は座標値ではなく画面上の距離として計算される。
    - 頂点をドラッグすると、その頂点を共有している面情報すべての形状が変化する。
    - コンテキストメニューで「共有を解除」を選ぶと、別々の頂点２つが同一座標に重なっている状態になる。
...
- **分裂と割譲/合邦（面情報の所属変更）**:
    ...
    - **割譲と合邦**
        - 面情報は、自分の上位領域を変更できる。
        - すべての面情報は、上位と下位を問わずあるあらゆる面情報に属しうる。
            - **ただし、自分自身に下位領域がない場合に限る。**
            - 例として、レベル1レイヤーの面情報でも連続していない遠い場所にあるレベル4レイヤーの面情報に属するレベル5レイヤーになることができる。
            - 逆に、レベル5であった面情報でもレベル1に昇格することが出来る。
            - 面情報は国の情報だけを記録するものではないが、国が基礎自治体になったり、基礎自治体が国に昇格することが可能。
        - 下位領域が既存ではない新しい上位領域に属することで、上位領域の分裂を表現する。
        - 下位領域すべてが別の上位領域に属することで、合邦を表現する。
...
- **編集モード**:
    ...
    - **コンテキストメニュー**: 頂点を右クリックすると開く。何が可能かは選択されているツールによって異なる。
    - 以下のツールが存在する。各ツールは自分の担当する種類のオブジェクトしか編集できない。
        - 例：点情報属性編集/頂点編集モードでは、線情報と面情報には反応しない。
        - **点情報属性編集**
        ...
        - **面情報頂点編集**
            ...
            - 同一レイヤーにある他の面情報の中に頂点をドラッグしていくと、エッジによって阻まれ、エッジに沿って滑るように動く。
                - マウスカーソルをモニターの端に動かした時に滑るようなイメージ。
            - 頂点が今まさに滑っている境界は、特別な色でハイライトされる。
            - エッジとエッジにも衝突判定が存在し、尖ったところを”飛び越える”ように動かしたとしても重ならない。
            - 同一レイヤーにある他の面情報の頂点に頂点を画面上の距離で近づけた場合、その頂点は共有頂点となる。
                - ただし、ドラッグ開始時の画面上の位置が近い場合、共有頂点にはならない。（直前に共有解除された場合を想定）
            - 共有頂点のコンテキストメニューで「共有を解除」を選ぶと、別々の頂点２つが同一座標に重なっている状態になる。
...
- トークン数の削減目的で英語を使わないでください。
    - これはコメントやデバッグログの翻訳禁止を意味します
- 断りなくコメントを削除・編集しないでください。
- 可能な限り編集する範囲を狭くしてください。

これらの指示を理解できたのなら、この「開発の仕方」セクションを復唱してください。

---

## **既知の不具合**

- 線情報頂点編集において、面のエッジに衝突が発生してしまう。
- 線情報追加において、既存の面内に頂点が作成できない
    - 本来線情報は面情報と干渉してはいけない。
- 頂点の選択機能が実装されているはずなのにできない
- 頂点の複数選択が実装されているはずなのにできない
- 点ドラッグが完全に機能していない
    - エッジハンドルへのクリックは効くが、ドラッグが全部駄目
- 共有頂点の仕組みは中途半端に実装されている。
    - 本来近づければ共有頂点化する実装がされているはずが、共有化されない。
    - 共有頂点の動作の検証が一切できていない
...
1. **頂点DB化と共有頂点の仕組み**
    ...
    2. **面の排他化** **完了**
        - データ構造を拡張し、レイヤーの概念を導入する。
        - 同じレイヤー内の面情報同士は、重なり合うことが出来ないように制約を追加。
        - 他の面情報の中に頂点をドラッグしていくと、エッジによって阻まれ、エッジに沿って滑るように動く
            - マウスカーソルをモニターの端に動かした時に滑るようなイメージ
        - エッジとエッジにも衝突判定が存在し、尖ったところを”飛び越える”ように動かしたとしても重ならないよう配慮
        - 頂点が今まさに滑っている境界のハイライトを追加
        - 面追加時、既存の面内に直接頂点を設置できないようにする。
    3. **頂点共有の導入** **イマココ！**
        - 点IDを２つ以上のオブジェクトで共有出来るようにする。この共有された頂点を共有頂点と呼ぶ。
        - 共有頂点は、既存にはない新しい色で塗る。
        - 共有頂点を作成する機能を追加（**画面上の距離が**近い場所に持っていくと吸い付く）**イマココ！**
        - 共有頂点のコンテキストメニューを実装し、そこで共有解除を可能にする。
        - コンテキストメニューのツールによる違いを実装する。
            - すべてのツールに共通として、コンテキストメニューからの頂点削除を可能にする。
            - 共有頂点のコンテキストメニューからのみ共有解除ができるように。
        - 共有解除された頂点同士（＝完全に重なった状態でドラッグが開始されたペア）は、共有頂点にならないようにする

2. **多層レイヤーの導入**
    1. **レイヤー概念の導入**
        - データ構造を拡張し、１枚以上のレイヤーを追加できるようにする。
        - レイヤーには序列がある。
        - すべてのオブジェクトはレイヤー１つだけに必ず属する。
        - 面情報は、「自分が属するよりも上位のレイヤー」（上位レイヤーと呼ぶ）に存在する面情報１つだけに必ず属する。
        - 面情報は、「上位レイヤーに存在する自分が属している面情報」（上位領域と呼ぶ）の範囲外にはみ出すことが出来ない（形状として必ず内包される）
